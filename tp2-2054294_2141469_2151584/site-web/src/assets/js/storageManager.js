...
export default class StorageManager {
  ...
  getItemById (storageKey, id) {
    ...
    const mydata= this.getData(storageKey);
    let mynewdata= mydata.find((item)=> {
      if(item.id===id){
        return item;
      }
    });
  ...
  addItem (storageKey, newItem) {  
    let newDataSt=this.getData(storageKey);
    newDataSt.push(newItem);
    localStorage.setItem(storageKey, JSON.stringify(newDataSt));
