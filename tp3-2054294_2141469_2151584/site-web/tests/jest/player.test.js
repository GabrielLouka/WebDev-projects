...
describe.only("Player tests", () => {
  ...
  it("loadSongs should load songs in playlist", () => {
    ...
    expect(player.songsInPlayList[0].src).toEqual(songStubs[0].src);
  });
  ...
  it("playPreviousSong should call playAudio", () => {
    ...
    const playerPlayPreviousSongSpy = jest.spyOn(player, "playPreviousSong");
    player.playPreviousSong();
    expect(playerPlayPreviousSongSpy).toBeCalled();
    expect(playerPlayAudioSpy).toBeCalled();
  });

  it("playNextSong should call playAudio", () => {
    const playerPlayAudioSpy = jest.spyOn(player, "playAudio").mockImplementation(() => {});
    const playerPlayNextSongSpy = jest.spyOn(player, "playNextSong");
    player.playNextSong();
  ...
  it("playPreviousSong should decrement currentIndex when shuffle is disabled", () => {
    ...
    player.loadSongs(songStubs);
    const currentIndexStub = 2;
    const expectedIndexStub = 1;
    player.currentIndex = currentIndexStub;
    player.playPreviousSong();
    expect(player.currentIndex).toEqual(expectedIndexStub);
  });

  it("playNextSong should increment currentIndex when shuffle is disabled", () => {
    jest.spyOn(player, "playAudio").mockImplementation(() => {});
    player.shuffle = false;
    const currentIndexStub = 1;
    const expectedIndexStub = 2;
    player.currentIndex = currentIndexStub;
    player.loadSongs(songStubs);
    player.playNextSong();
    expect(player.currentIndex).toEqual(expectedIndexStub);
  ...
  it("muteToggle should return true if audio is muted", () => {
    const volumeStub = 0;
    player.audio.volume = volumeStub;
    expect(player.muteToggle()).toEqual(true);
  });
  ...
  it("shuffleToggle should inverse shuffle state", () => {
    const inverseShuffleStateStub = !player.shuffle;
    expect(player.shuffleToggle()).toEqual(inverseShuffleStateStub);
  });
