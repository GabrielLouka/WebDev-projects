...
class PlaylistService {
  ...
  async updatePlaylist (playlist) {
    ...
    const setQuery = {$set:{id:playlist.id, name:playlist.name, 
    description:playlist.description,thumbnail:playlist.thumbnail, songs:playlist.songs }};
  ...
  async search (substring, exact) {
    ...
    if(exact){
      const playlists = await this.collection.find({$or:[filterSensitiveCaseName,filterSensitiveCaseDescription]}).toArray();
      return playlists
    }
    const playlists = await this.collection.find({$or:[filterName,filterDescription]}).toArray();
    return playlists;
  }
